  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/recruiters/signup', {
        ...
        body: JSON.stringify({ email, password, name, agency }),
      });
      ...
      toast({ title: `Welcome to ${config.title}`, description: 'Your recruiter account is ready.' });
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="font-medium text-sm" htmlFor="name">Full name *</Label>
            <Input
          ...
          <div className="space-y-2">
            <Label className="font-medium text-sm" htmlFor="agency">
              Agency / Company <span className="text-zinc-400 font-normal">(optional)</span>
            </Label>
            <Input
              disabled={isSubmitting}
              id="agency"
              onChange={(e) => setAgency(e.target.value)}
              placeholder="e.g. TalentBridge Recruiting"
              type="text"
              value={agency}
            />
          </div>
